...
const CharacterInfoModal = ({ show, onClose, characterName, characterDescription, characterHeight }) => {
  return (
...
const NewspaperModal = ({ show, onClose }) => {
  ...
  const handleCharacterClick = (name, description, height) => {
    setCharacterInfo({
      name,
      description,
      height
  ...
  return (
    <Modal show={show} onHide={onClose} size="xl">
        ...
        <Container>
          <Row className='mx-2 row row-cols-3'>
            {characters.map((character, index) => (
              <Card className="character-card" key={index} onClick={() => handleCharacterClick(character.name, character.description, character.height)}>
      ...
      <CharacterInfoModal
        ...
        onClose={() => setCharacterInfo({ name: '', description: '', height:'' })}
